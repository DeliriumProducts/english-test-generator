...
namespace English_Test_Generator
{
    class SearchWord
    {
        public static string GetCorrectWord(string word_id, string region)
        {
            ...
            if (TestGenerator.word_prev == word_id) return TestGenerator.word_id; // exits the method if the entered word by the user mathes the previous word so no unnecessary requests are made
            ...
            {     
                ...
                if (word_id != TestGenerator.word_id && word_id != "Word not found" && result.Results.Length>=1) // checks if the first result doesn't match the user's word nor is equal to "Word not found" and if there's more than one result   
                {                                                          
                    for (int i = 0; i < result.Results.Length; i++) // starts going through each result
                    {
                        if (TestGenerator.word_id.ToLower() == result.Results[i].Word) // check if the current result matches the user's word
                        {                          
                            return TestGenerator.word_id; // returns the user's word
                        }
                        TestGenerator.fr.comboBox3.Items.Add(result.Results[i].Word); // starts filling comboBox3's items with the results
                    }
                    TestGenerator.fr.comboBox3.Visible = true; // makes comboBox3 visible
                    TestGenerator.fr.comboBox3.Text = "";
