...
namespace GetAPIResponse
{
    ...
    class Definitions
    {
        ...
        {
            string filter = (lexicalCategory=="") ? "noun,adjective,verb,adverb,idiomatic" : lexicalCategory;
            string definitions ="";
            string url = "https://od-api.oxforddictionaries.com:443/api/v1/entries/en/" + word + "/lexicalCategory=" + filter +";definitions;regions=" + Form1.region; 
                ...
                {                
                    ...
                    {                        
                        for (int k = 0; k < result.Results.First().LexicalEntries[i].Entries[j].Senses.Length; k++) // k = all definitions from the API response 
                        {
                            ...
                            if (result.Results.First().LexicalEntries[i].Entries[j].Senses[k].Subsenses !=null) // checks if there's(re) any subsenses in the current sense 
                            {
